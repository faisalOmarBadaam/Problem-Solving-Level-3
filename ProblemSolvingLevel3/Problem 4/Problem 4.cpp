...
void printArrayCloumnSum(int arr[100][100], int columns, int rows) {
	int sum = 0;
	for (int i = 0; i < rows; i++)
	{
		sum = sumColmuns(arr,columns,i);
		cout << "sum of the " << i + 1 << "st column is : " << sum<<endl;
	}
}
int main()
{
	...
	printArrayCloumnSum(arr, lenthC, lenthR);
